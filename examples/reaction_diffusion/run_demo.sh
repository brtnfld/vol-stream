...
find_bin() {
    local name="$1"
    for d in "$BUILD_DIR" "$SCRIPT_DIR" "$SCRIPT_DIR/../../build/examples/reaction_diffusion" \
             "$SCRIPT_DIR/../../build-nomercury/examples/reaction_diffusion"; do
...
# the terminal heatmap from rd_monitor is the demo either way. gnuplot's
...
# shape before starting it -- rd_monitor overwrites it within its first
